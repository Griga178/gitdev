...
class Subject_ver_3():
    ...
    def chars_description_dict(self, examples_set, first_rec = True):
        # if first_rec:
            ...
            for child_name in self.child:
                for example in examples_set:
                    if child_name == example.name:
                        child_example = example
                        break
                ex_desc_d[self.name]['content'].update(child_example.chars_description_dict(examples_set, first_rec))
        else:
